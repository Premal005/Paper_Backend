...
def get_fyers_symbol_format(symbol: str, instrument_type: str = "EQ") -> str:
    ...
    Formats:
    - Equity: NSE:SYMBOL-EQ
    - NSE Futures: NSE:SYMBOLYYMMFUT
    - NSE Options: NSE:SYMBOLYYMMSTRIKEPE/C
    - MCX Futures: MCX:SYMBOLYYMMFUT
    - MCX Options: MCX:SYMBOLYYMMSTRIKEPE/C
    """
    symbol = symbol.upper().strip()
    
    if instrument_type == "FUTURES":
        # Determine exchange based on symbol
        if any(commodity in symbol for commodity in ["GOLD", "SILVER", "COPPER", "CRUDEOIL", "NATURALGAS"]):
            return f"MCX:{symbol}FUT"  # MCX Futures
        else:
            return f"NSE:{symbol}FUT"  # NSE Futures
    
    elif instrument_type == "OPTIONS":
        # Determine exchange based on symbol
        if any(commodity in symbol for commodity in ["GOLD", "SILVER", "COPPER", "CRUDEOIL", "NATURALGAS"]):
            return f"MCX:{symbol}"  # MCX Options
        else:
            return f"NSE:{symbol}"  # NSE Options
...
def onmessage(message):
    ...
    try:
        ...
        # Extract symbol and determine instrument type and exchange
        symbol = message.get("symbol", "")
        instrument_type = "EQ"  # Default to equity
        exchange = "NSE"  # Default to NSE
        
        # Detect exchange and instrument type from symbol
        ...
        df = pd.DataFrame([data])
        df.to_csv(csv_file, mode="a", header=not os.path.exists(csv_file), index=False)
        # logger.info(f"📈 {data['symbol']} | LTP: {data['ltp']} | Exchange: {exchange} | Type: {instrument_type}")
...
def start_fyers_feed(broadcast=None, main_loop=None, subscribed_symbols=None):
    ...
    if not subscribed_symbols:
        subscribed_symbols = [
            ...
            "NSE:TCS-EQ",
            # NSE Futures
            "NSE:BANKNIFTY24SEPFUT",
            "NSE:NIFTY24SEPFUT",
            # NSE Options  
            "NSE:BANKNIFTY2422945000CE",
            "NSE:BANKNIFTY2422945000PE",
            # MCX Futures
            "MCX:GOLD25DECFUT",
            "MCX:SILVER25DECFUT",
            # MCX Options
            "MCX:GOLD25DEC60000CE",
            "MCX:GOLD25DEC60000PE",
        ]
    ...
    try:
        ...
        if ":" not in symbol and "-" not in symbol:
            ...
            for possible_symbol in possible_symbols:
                doc = await market_data.find_one({"symbol": possible_symbol})
                if doc:
